"""
Wrapper functions for the conventional DNN network
"""
# Built-in
import os
import time
...
class Network(object):
    def __init__(self, model_fn, flags, train_loader, test_loader,
        ...
        self.best_mse_loss = float('inf')           # Sometimes a loss function other than MSE is used
    ...
    def make_custom_loss(self, logit=None, labels=None):
        """
        Two custom loss functions, one using real/imaginary parts of r,t as loss terms, and the other using R,T
        """
        if logit is None:
            return None
    ...
    def make_lr_scheduler(self):
        """
        Make the learning rate scheduler as instructed. More modes can be added to this, currently supported:
    ...
    def init_weights(self):
        """
        Can change how the network weights are initialized here. Currently defaults to xavier uniform init.
        :return: None
        """
    ...
    def evaluate(self, save_dir='eval/'):
        ...
        :return: truth and prediction files from model under evaluation
        """
        ...
        with open(Xtruth_file, 'a') as fxt, open(Ytruth_T_file, 'a') as fyt_1, open(Ypred_T_file, 'a') as fyp_1, \
            ...
            with torch.no_grad():
                for ind, (geometry, spectra) in enumerate(self.test_loader):
                    ...
                    # Calculate R,T from predicted scattering parameters r,t
        ...
        # Evaluates MSE on the predicted transmission spectra (can also choose R instead)
    ...
    def train(self):
        ...
        # Start a tensorboard session for logging loss and training performance
        ...
        for epoch in range(self.flags.train_step):
            ...
            for j, (geometry, spectra) in enumerate(self.train_loader):
                ...
                # Can switch between using either MSE loss or custom loss function
                # loss = self.make_MSE_loss(logit.float(), square(abs(spectra[:,:,1])).float())
                loss = self.make_custom_loss(logit, spectra)

                # Saves image of network graph at first training epoch
                # if j == 0 and epoch == 0:
                ...
                # Clip gradients to help with training if desired, either grad value or norm
                ...
                # Plots comparing DNN predictions to ground truth during training, viewed on tensorboard
            ...
            # Reset lr if warm restarts are used
            if epoch > 10:
