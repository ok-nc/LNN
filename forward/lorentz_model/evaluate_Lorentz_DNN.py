...
def normalize(params, geoboundary):
    num_params = len(params)
    for p in range(num_params):
        params[p] = params[p] * 0.5 * (geoboundary[p+num_params] - geoboundary[p]) + (
                    geoboundary[p+num_params] + geoboundary[p]) * 0.5
...
if __name__=='__main__':
    ...
    # geom = np.array([1.5,2.8,7,43])
    #
    # lorentzian_osc_from_model(model_name, w, geom, save_dir='CST_Comp', save_name='LNNpred_'+str(geom),
    #                               save_spectra=True, save_params=True, save_osc=True)


    for i in range(21):
        # geom = np.array([1.3 + i * 0.1, 1.8, 7, 42])
        geom = np.array([1.9, 1.0 + i * 0.1, 7, 42])
        # geom = np.array([1.9, 1.72, 6 + i * 0.04, 43.5])
        # geom = np.array([1.3 , 2.8, 7, 40+ i * 1])
        geometry = np.round(geom, 3).astype(str)
        geo_str = '['
        for k in range(geometry.shape[0]):
            geo_str += str(geometry[k])+' '
        geo_str += ']'
        save_dir = 'r_sweep_wp_eps_r1.9'
        sweep_name = save_dir + '_' + geo_str
        lorentzian_osc_from_model(model_name, w, geom, save_dir=save_dir, save_name=sweep_name,
                                  save_spectra=True, save_params=True, save_osc=True)
    lor_parameter_sweep_analysis(3, model_name, save_dir, save_name=save_dir + '_params')

    geom = np.array([1.3, 1, 7, 42])
    sweep_num = 21

    for i in range(sweep_num):
        geom[0] = 1.3 + i * (0.03)
        # geom[3] = 40.5 + i * (0.035)
        save_directory = '2D_sweep_' + 'r_h_sweep_' + str(i)
        for k in range(sweep_num):
            # geom[1] = 2 / (1.15 + k * 0.0085)
            # geom[3] = 44 - k * (0.01)
            geom[1] = (1.0 + k * 0.1)
            geom[1] = np.round(geom[1],1)
            geometry = np.round(geom,1).astype(str)
            ...
            lorentzian_osc_from_model(model_name, w, geom, save_dir=save_directory, save_name='rh_S_'+geo_str,
                                      save_spectra=True, save_params=True, save_osc=True)
