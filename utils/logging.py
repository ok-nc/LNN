...
def save_flags(flags, save_dir, save_file="flags.obj"):
    """
    This function serializes the flag object and saves it for further retrieval during inference time
...
def load_flags(save_dir, save_file="flags.obj"):
    """
    This function inflates the pickled object to flags object for reuse, typically during evaluation (after training)
...
def evaluate_mse_on_dataset(truth_file, pred_file):

    df1 = pd.read_csv(truth_file, delimiter='\s', engine='python')
    df2 = pd.read_csv(pred_file, delimiter='\s', engine='python')
    mse = ((df1.values - df2.values) ** 2).mean(axis=1).mean(axis=0)
