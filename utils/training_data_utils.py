"""
This file contains processing functions for importing ascii data and generating Pytorch training datasets
"""
...
# Function to compress all CST output to single hdf5. Options to select which data channels and various dataset params
...
# Checks distribution of input parameters to make sure geometry points are sampled uniformly
...
# Imports training data files in csv format
...
def generate_torch_dataloader(x_range, y_range, geoboundary, normalize_input=True,
    ...
    """
    Generates Pytorch training and testing datasets using Dataset subclass
    ...
    # Can downselect from complete training dataset here for specific parameter subranges
    ...
    if (test_ratio > 0):
        ...
        s11 = np.expand_dims(s11_re + 1j * s11_im, axis=2)  # The different signs here are to account for
        s21 = np.expand_dims(s21_re - 1j * s21_im, axis=2)  # correct phase convention in CST port definitions
